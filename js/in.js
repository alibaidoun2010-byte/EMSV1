...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  await trackUniqueUser();
  await incrementUserCountOnRefresh();
...
async function updateRestaurantLogos() {
  try {
    console.log('بدء تحديث صور اللوغو من مجلد المشروع المحلي...');

    // مسار اللوغو الموجود في مجلد مشروعك
    const localLogoPath = 'assets/images/garden.jpg';
    // المسار الافتراضي الاحتياطي في حال فشل التحميل
    const defaultFallbackLogo = 'assets/images/garden.jpg';

    // قائمة بجميع عناصر اللوغو التي تحتاج للتحديث
    const logoImages = [
    ...
    logoImages.forEach(({ id }) => {
      const img = document.getElementById(id);
      if (img) {
        console.log(`تحديث الصورة ${id} إلى المسار المحلي:`, localLogoPath);
        img.src = localLogoPath;

        // معالجة حالة الخطأ (في حال كان الملف غير موجود)
        img.onerror = function() {
          console.warn(`فشل تحميل الصورة من المسار المحلي لـ ${id}، استخدام المسار الاحتياطي`);
          this.src = defaultFallbackLogo;
    ...
    console.log(`تم توجيه ${logoImages.length} صورة إلى المسار المحلي`);
...
async function trackUniqueUser() {
  // 1. التحقق من العلامة المحلية في المتصفح
  const hasVisited = localStorage.getItem('has_visited_site');
  if (hasVisited) {
    console.log('المستخدم زار الموقع سابقاً من هذا المتصفح، لن يتم زيادة العداد.');
    return;
  }

  try {
    // 2. جلب الصف الأول مع الـ id والـ users من جدول info
    ...
    if (fetchError) {
      console.error('خطأ Supabase في جلب البيانات:', fetchError);
      return;
    }

    if (!data) {
      console.error('جدول info فارغ أو لا يحتوي على صفوف!');
      return;
    }

    const currentUsers = Number(data.users || 0);

    // 3. التحديث بناءً على الـ id الخاص بالصف
    ...
    if (!updateError) {
      // 4. حفظ العلامة محلياً لمنع التكرار مستقبلاً
      localStorage.setItem('has_visited_site', 'true');
      console.log(`تمت زيادة العداد بنجاح! القيمة الجديدة: ${currentUsers + 1}`);
    } else {
      console.error('خطأ Supabase في التحديث (تأكد من صلاحيات UPDATE):', updateError);
    }
  } catch (err) {
...
async function incrementUserCountOnRefresh() {
  try {
    // 1. جلب الصف الأول مع الـ id والـ users من جدول info
    const { data, error: fetchError } = await supabase
      .from('info')
      .select('id, usings')
      .limit(1)
    ...
    const currentUsings = Number(data.usings || 0);

    // 2. تحديث العداد وزيادته بمقدار 1 في كل تحديث للموقع
    const { error: updateError } = await supabase
      .from('info')
      .update({ usings: currentUsings + 1 })
      .eq('id', data.id);

    if (!updateError) {
      console.log(`تمت زيادة العداد بنجاح! القيمة الجديدة: ${currentUsings + 1}`);
    } else {
      console.error('خطأ Supabase في التحديث (تأكد من صلاحيات UPDATE):', updateError);
    }
  } catch (err) {
...
// عدّل الروابط التالية إلى صفحات المطعم الرسمية
const INSTAGRAM_URL = 'https://www.instagram.com/garden';
const FACEBOOK_URL  = 'https://www.facebook.com/garden';
...
/* ===================== شريط الصور المتحرك (Supabase Storage) ===================== */
const SLIDER_BUCKET = 'slide_bar';
...
async function loadSliderImages() {
  ...
  try {
    // جلب الصفوف التي تحتوي على رابط الصورة من جدول slider_imgs
    const { data, error } = await supabase
      .from('slider_imgs')
      .select('url')
      .order('id', { ascending: true }); // يمكنك الترتيب حسب id أو created_at حسب مجالات الجدول لديك

    if (error) {
      console.error('خطأ في جلب صور السلايدر من جدول slider_imgs:', error.message || error);
      return;
    }

    // فلترة النتائج للتأكد من وجود رابط صحيح غير فارغ
    const validRows = (data || []).filter(item => item && item.url && item.url.trim() !== '');

    if (!validRows.length) {
      console.warn('لا توجد صور في جدول slider_imgs.');
      return;
    }
    ...
    if (validRows.length > 1) startSliderAutoplay();

  } catch (err) {
    console.error('خطأ غير متوقع في تحميل السلايدر:', err);
  }
}
